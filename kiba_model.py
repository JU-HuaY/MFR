...
class CNN_MLP(nn.Module):
    def __init__(self, Affine, patch, channel, output_size, dr, down=False, last=False):
        ...
        self.cnn1 = nn.Conv1d(in_channels=patch, out_channels=patch, kernel_size=23, padding=11, groups=patch)
        self.bn1 = nn.BatchNorm1d(patch)
        self.cnn2 = nn.Conv1d(in_channels=patch, out_channels=patch, kernel_size=35, padding=17, groups=patch)
...
class ResMLP(nn.Module):
    def __init__(self, Affine, patch, channel, output_size, dr, down=False, last=False):
        ...
        self.cross_channel_linear1 = nn.Linear(channel, channel)
        self.cross_channel_linear2 = nn.Linear(channel, channel)
        #self.cross_channel_linear3 = nn.Linear(channel, channel)
    ...
    def forward(self, x):
        ...
        x_cc2 = self.activation(self.cross_channel_linear2(x_cc2))
...
class RWingLoss(nn.Module):
    ...
    def forward(self, pred, tar):
        ...
        loss2 = de_y2 - C
        return 0.1 * (loss0.sum() + loss1.sum() + loss2.sum()) / (len(loss0) + len(loss1) + len(loss2))
...
class cross_layers(nn.Module):
    def __init__(self, channel, length, attention_dropout=0.05):
        ...
        self.cross_atten1 = cross_att(channel, 1301)
        self.cross_atten2 = cross_att(channel, 1301)
        self.dropout = nn.Dropout(attention_dropout)
        self.linear = nn.Linear(150, 150)
        self.maxpool1 = nn.AdaptiveAvgPool1d(1)
        self.maxpool2 = nn.AdaptiveAvgPool1d(1)
        self.downsample2 = nn.Linear(75, 1)
        self.act = nn.ReLU()
...
class Mix_Decoder(nn.Module):
    ...
    def __init__(self, channel, patch, attention_dropout=0.1):
        ...
        # self.self_layers = self_layers(channel, patch, attention_dropout)
        ...
        self.sample_a = nn.Linear(101, 101)
        self.sample1 = nn.Linear(101, 75)
...
class Predictor(nn.Module):
    def __init__(self, Decoder, ResMLP, Affine, dim, window, window2, layer_gnn, layer_cnn, layer_output):
        ...
        # self.WP_NN0 = CNN_MLP(Affine, 1200, 100, 75, 0, True)
        self.WP_NN1 = CNN_MLP(Affine, 1200, 100, 75, 0, True)
        self.WP_NN2 = CNN_MLP(Affine, 1200, 100, 75, 0, True, True)

        # self.WP_FiNN0 = CNN_MLP(Affine, 1200, 100, 75, 0, True)
        self.WP_FiNN1 = CNN_MLP(Affine, 1200, 100, 75, 0, True)
        self.WP_FiNN2 = CNN_MLP(Affine, 1200, 100, 75, 0, True, True)

        self.WC_struc1 = ResMLP(Affine, 100, 75, 75, 0)
        #self.WC_struc3 = ResMLP(Affine, 100, 75, 75, 0)
        self.WC_struc2 = ResMLP(Affine, 100, 75, 75, 0, False, True)  # 1

        self.WC_fcfp1 = ResMLP(Affine, 1, 2048, 75, 0)
        self.WC_fcfp3 = ResMLP(Affine, 1, 2048, 75, 0)
        self.WC_fcfp2 = ResMLP(Affine, 1, 2048, 75, 0, False, True)

        # self.WC_struc2 = ResMLP(Affine, 50, 75, 75, 0) #4

        self.WPDI_len1 = ResMLP(Affine, 75, 600, 100, 0)
        # self.WPDI_len2 = ResMLP(Affine, 75, 1300, 2, 0, True)

        self.WP_fixfea = ResMLP(Affine, 75, 2400, 1200, 0, False, True)
        ...
        self.W_out1_1 = nn.Linear(1301, 1024)
        ...
        self.dropout1_2 = nn.Dropout(p=0.05)
        self.activation = nn.ReLU()
        self.activation2 = nn.GELU()
        self.act_norm = Swish(True)
        self.act_norm2 = Swish(True)
        self.act_norm3 = Swish(False)
        self.sigmoid = nn.Sigmoid
        ...
        nn.init.xavier_normal_(self.W_interaction2.weight)
    ...
    def attention_PM2(self, xs, x):
        ...
        scale = weights.size(-1) ** -0.5
        # ys = torch.matmul(weights, h)
        ys = self.softmax2(torch.matmul(weights, h) * scale) * xs
        return ys
    ...
    def forward(self, inputs):
        ...
        compound_vector = torch.zeros((N, 100, 75), device=device)
        ...
        # print(compound_vec.shape)
        if t <= 100:
            compound_vector[:, 0:t, :] = compound_vec
        else:
            compound_vector = compound_vec[:, 0:100, :]
        ...
        mol_vector = mol_vector + self.WC_fcfp3(mol_vector)
        mol_vector = self.WC_fcfp2(mol_vector)


        compound_vectors = self.attention_PM(compound_vector, mol_vector)  # .permute(0, 2, 1)
        mol_vectors = self.attention_PM2(mol_vector, compound_vector)
        ...
        adjacencys = torch.zeros((N, 101, 101), device=device)
        if L > 101:
            adjacencys = adjacency[:, 0:101, 0:101]
        else:
            adjacencys[:, 0:L, 0:L] = adjacency

        dta1, dta2, response = self.Decoder(protein_vectors, drug_vectors, adjacencys)

        PDI1 = self.W_out1_1(dta1)
        PDI1 = self.dropout(PDI1)
        PDI1 = self.activation(self.W_out1_2(PDI1))
        PDI1 = self.activation(self.W_out1_3(PDI1))

        PDI2 = self.activation(self.W_out2_1(dta2))
        # PDI2 = self.dropout(PDI2)
    ...
    def __call__(self, data, epoch, train=True):
        # print(np.array(data).shape)
        inputs, correct_interaction, res_labels = data[:-2], data[-2], data[-1]
        # correct_interaction = torch.nn.functional.one_hot(correct_interaction, 2)
        # labels2 = labels
        correct_interaction = correct_interaction
        # samples, mse = label_conce(correct_interaction)

        protein_drug_interaction, response = self.forward(inputs)
        # print(apha.to('cpu').data.numpy()[0])
        ...
        # Mmsamples = Mmsamples.squeeze(1)
        # print(mse)
        # correct_interactions = correct_interaction.to(torch.float32).unsqueeze(1).expand(interactions.to(torch.float32).size())
        # print(torch.max(res_labels))
        if train:

            criterion = nn.MSELoss()  # nn.SmoothL1Loss() # SmoothL1Loss()RWingLoss(om=1, ep=2, r=0.1)#
            # criterion_res = nn.SmoothL1Loss()
            criterion_res = RWingLoss(om=1, ep=3, r=0.15)

            # criterion_sample = RWingLoss(om=1, ep=3, r=0.5)#nn.MSELoss()#.CrossEntropyLoss()
            ...
            # loss4 = criterion_res(dta2.to(torch.float32), dta1.to(torch.float32))
            # print(loss3)

            return loss1 + loss2, loss1
        else:
            correct_labels = correct_interaction.to('cpu').data.numpy()
            ys = protein_drug_interaction.to('cpu').data.numpy()
            predicted_scores = ys  # list(map(lambda x: x[0], ys))
    ...
    def train(self, dataset, epoch):
        ...
        for data in dataset:
            ...
            if i % self.batch == 0 or i == N:
                ...
                loss, loss3 = self.model(data, epoch)
    ...
    def test(self, dataset):
        ...
        for data in dataset:
            ...
            if i % 16 == 0 or i == N:
                ...
                # print(words.shape)
                data = (fingerprints, compounds_words, adjacencys, morgans, words, protein_sss, labels, res_labels)
                (correct_labels, predicted_scores) = self.model(data, epoch, train=False)
...
def dataset(dir_input1, dir_input2):
    ...
    compounds1 = load_tensor(dir_input1 + 'compounds', torch.FloatTensor)
    compounds_word1 = load_tensor(dir_input1 + 'compound_words', torch.FloatTensor)
    adjacencies1 = load_tensor(dir_input1 + 'adjacencies', torch.FloatTensor)
    morgans = load_tensor(dir_input1 + 'morgan', torch.FloatTensor)
    proteins1 = load_tensor(dir_input1 + 'proteins_feature', torch.FloatTensor)
    protein_ss1 = load_tensor(dir_input1 + 'proteins', torch.FloatTensor)
    ...
    compounds2 = load_tensor(dir_input2 + 'compounds', torch.FloatTensor)
    compounds_word2 = load_tensor(dir_input2 + 'compound_words', torch.FloatTensor)
    adjacencies2 = load_tensor(dir_input2 + 'adjacencies', torch.FloatTensor)
    morgans2 = load_tensor(dir_input2 + 'morgan', torch.FloatTensor)
    proteins2 = load_tensor(dir_input2 + 'proteins_feature', torch.FloatTensor)
    protein_ss2 = load_tensor(dir_input2 + 'proteins', torch.FloatTensor)
...
if __name__ == "__main__":
    ...
    # setting) = sys.argv[1:]
    DATASET = "/home/zhong/data_davis_kiba/DTA_data/kiba_input/1"
    radius = 2
    ngram = 3
    dim = 75
    layer_gnn = 3
    ...
    model = model.cuda()
    # model.load_state_dict(torch.load("output/model/kiba_ori_11"))
    ...
    for epoch in range(1, iteration):
        ...
        loss_train, loss_total2 = trainer.train(dataset_train, epoch)
        ...
        np_t = 'output/np/kiba' + setting + str(epoch) + '_t.npy'
        np_s = 'output/np/kiba' + setting + str(epoch) + '_s.npy'
        if ci < CI:
            ci = CI
